...
def plot_3d_clustering_animated(df: pd.DataFrame, labels, title: str, n_frames: int = 36):
    """Create slow rotating 3D visualization"""
    numeric_df = df.select_dtypes(include=[np.number])
                ...
                if should_run:
                    with st.spinner("🤖 Agent analyzing clustering options..."):
                        try:
                            ...
                            if workflow_state.get('workflow_status') in ['ready_for_approval', 'ready_for_review']:
                                st.success("✓ Clustering complete! Variants generated.")
                            else:
                                st.error(f"Clustering failed: {workflow_state.get('error_message')}")
                                ...
                                if i + col_idx < len(variant_list):
                                    ...
                                    with col:
                                        ...
                                        if 'labels' in variant_data:
                                            st.write("**3D Rotating View:**")
                                            fig_3d_anim = plot_3d_clustering_animated(df, variant_data['labels'], f"{name}")
                                            st.plotly_chart(fig_3d_anim, width='stretch')
